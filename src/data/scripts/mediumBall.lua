...
function MediumBall:create()
	go = GameObjectManager:createGameObject("MediumBall" .. self.uniqueIdentifier)
	...
	rawset(balls, "MediumBall" .. self.uniqueIdentifier, self)
	
	logMessage("mediumBall created")
	
	objectManager:addPool(SmallBall, 2)
...
function MediumBall:update()
		...
		ball1:setInitialMovement(self.go.getPosition()+Vec3(0,0.76,0), Vec3(0, 2.0, 0.0))
		local ball2 = objectManager:grab(MediumBall)
		ball2:setInitialMovement(self.go.getPosition()+Vec3(0,-0.76,0), Vec3(0, -2.0, 0.0))
